...
✅ **Do:** Pick the right primitive for the job.
```tsx
// Full card when you need header/body/footer structure
<Card>
  <CardHeader>
...
<Stack gap="md">
  <Heading level={3}>Inline form</Heading>
...
**Decision guide**
- Use **Card** for discrete blocks that benefit from header/body/footer or need a defined surface. Prefer `variant="plain"` or `variant="ghost"` when you don't want a border or filled background.
- Use **Panel** for lightweight grouping and background treatment without Card chrome. Pick `variant="plain"` (no border) or `variant="ghost"` (transparent) for most in-flow sections.
- Use **Stack**, **Inline**, **Grid**, and **Container** to handle spacing and layout instead of custom margins/gaps on raw divs.
...
| `Panel` | `@/components/ui/panel` | Light grouping without heavy chrome |
| `PageHeader` | `@/components/layout` | Page headers |

### Surfaces and when to use them
| Primitive | When to use | Notes |
|-----------|-------------|-------|
| `Card` | Discrete content block that needs header/body/footer, separation, or selectable surface | Use `variant="plain"` or `variant="ghost"` when you need the structure but not the border/background |
| `Panel` | Grouping content inside flows without Card chrome | `variant="plain"` (no border) is the default for neutral grouping; `bordered`/`elevated` when you need subtle separation |
| `Stack` / `Inline` | Control spacing and alignment without adding a surface | Prefer these over manual `gap`/`space-y`/`flex` classes |
| `Container` | Constrain content width and add page padding | Use for page bodies and large sections |
