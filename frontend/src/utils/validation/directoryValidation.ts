...
export const directoryValidation = {
  ...
  validatePhone: (phone: string): ValidationResult => {
    ...
    if (digitsOnly.length < 10) {
      return { error: 'Phone number must be at least 10 digits', isValid: false };
    }
  ...
  validateCompanyName: (name: string): ValidationResult => {
    ...
    if (name.trim().length < 2) {
      return { error: 'Company name must be at least 2 characters', isValid: false };
    }

    if (name.length > 100) {
      return { error: 'Company name must be less than 100 characters', isValid: false };
    }
  ...
  validateUserName: (firstName: string, lastName: string): ValidationResult => {
    ...
    if (firstName.length > 50) {
      return { error: 'First name must be less than 50 characters', isValid: false };
    }

    if (lastName.length > 50) {
      return { error: 'Last name must be less than 50 characters', isValid: false };
    }
  ...
  validateLicenseNumber: (licenseNumber: string): ValidationResult => {
    ...
    if (licenseNumber.length < 3) {
      return { error: 'License number must be at least 3 characters', isValid: false };
    }

    if (licenseNumber.length > 50) {
      return { error: 'License number must be less than 50 characters', isValid: false };
    }
   ...
   */
  validateUniqueEmail: async (email: string, projectId: string): Promise<ValidationResult> => {
   ...
   */
  validateUniqueCompanyName: async (name: string, projectId: string): Promise<ValidationResult> => {
    const supabase = createClient();
    ...
    if (data) {
      return { error: 'A company with this name already exists', isValid: false };
    }
  ...
  validateJobTitle: (jobTitle: string): ValidationResult => {
    ...
    if (jobTitle.length > 100) {
      return { error: 'Job title must be less than 100 characters', isValid: false };
    }
...
export function validateFields(
  validations: Array<{ validate: () => ValidationResult | Promise<ValidationResult>; field: string }>
  ...
  return Promise.all(
    ...
    return results.reduce((acc, { field, error }) => {
      if (error) {
        acc[field] = error;
      }
      return acc;
