...
describe('CompanyService', () => {
  ...
  describe('getCompanies', () => {
    ...
    const createChainableMock = (resolvedValue: { data: unknown; error: unknown; count?: number }) => {
      ...
      const methods = ['select', 'eq', 'order', 'range', 'ilike', 'or', 'single'];
    ...
    it('should return paginated list of companies', async () => {
      ...
      const chainable = createChainableMock({ data: mockData, error: null, count: 2 });
      ...
      jest.spyOn(service as never, 'getCompanyUserCount').mockResolvedValue(0 as never);
    ...
    it('should filter by status', async () => {
      const chainable = createChainableMock({ data: [], error: null, count: 0 });
      const mockFrom = jest.fn().mockReturnValue(chainable);
  ...
  describe('updateCompany', () => {
    it('should update both global and project-specific fields', async () => {
      ...
      expect(updateCalls.some(c => c.table === 'project_companies')).toBe(true);
    });
  });
  ...
  describe('canDeleteCompany', () => {
    ...
    it('should return canDelete: false when users are assigned', async () => {
      jest.spyOn(service as never, 'getCompanyUserCount').mockResolvedValue(5 as never);
