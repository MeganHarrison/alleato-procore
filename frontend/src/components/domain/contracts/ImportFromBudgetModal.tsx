...
export function ImportFromBudgetModal({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to import from budget';
    ...
    <Dialog open={open} onOpenChange={isImporting ? undefined : onOpenChange}>
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Select budget line items to import into the contract schedule of values
          </DialogDescription>
        </DialogHeader>
                    ...
                    <tr>
                      <th className="px-4 py-3 text-left w-12">
                        <Checkbox
                          checked={
                            filteredLines.length > 0 &&
                            filteredLines.every(line => selectedIds.has(line.id))
                          }
                  ...
                  <tbody>
                    {filteredLines.map((line) => (
                      <tr
                        ...
                        onClick={() => handleSelectLine(line.id, !selectedIds.has(line.id))}
                      >
