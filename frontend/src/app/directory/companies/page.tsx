  ...
  const columns: EditableColumn<CompanyData>[] = [
    {
      ...
      render: (value) => (
        <div className="flex items-center gap-2">
          ...
          <Text as="span" weight="medium">{value || 'Unnamed Company'}</Text>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      type: 'text',
      render: (value, row) => {
        if (!value && !row.city && !row.state) return <Text as="span" tone="muted">-</Text>;
        const parts = [value, row.city, row.state].filter(Boolean);
        return <Text as="span" size="sm">{parts.join(', ')}</Text>;
      },
    },
    {
      key: 'city',
    ...
    },
    {
      ...
      render: (value, row) => value ? (
        <Text as="span" size="sm">{row.currency_symbol || ''}{value}</Text>
      ) : <Text as="span" tone="muted">-</Text>,
    },
    {
      key: 'notes',
      ...
      type: 'textarea',
      render: (value) => value ? (
        <Text as="span" size="sm" tone="muted" className="line-clamp-2">{value}</Text>
      ) : <Text as="span" tone="muted">-</Text>,
    },
  ];
            ...
            <div>
              {isSuccess && (
                <Text as="p" size="sm" tone="muted">
                  <Text as="span" weight="medium">{data.length}</Text> of <Text as="span" weight="medium">{count}</Text> companies loaded
                </Text>
              )}
            </div>
          </div>
          ...
          <div>
            {isLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <Text tone="muted" className="mb-4">Get started by adding your first company.</Text>
                <Button onClick={handleAddCompany} variant="default">
                  <Plus className="mr-2 h-4 w-4" />
                  Add Company
                </Button>
