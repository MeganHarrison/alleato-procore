...
interface RouteParams {
  params: Promise<{ id: string; contractId: string; lineItemId: string }>;
}
...
export async function GET(
  ...
  try {
    const { id: projectId, contractId, lineItemId } = await params;
    ...
    console.error('Error in GET /api/projects/[id]/contracts/[contractId]/line-items/[lineItemId]:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    if (!projectMember || !['editor', 'admin', 'owner'].includes(projectMember.access)) {
      return NextResponse.json(
        { error: 'Forbidden: You do not have permission to update line items for this project' },
        { status: 403 }
      );
    }
    ...
    if (validatedData.line_number && validatedData.line_number !== existingLineItem.line_number) {
    ...
    console.error('Error in PUT /api/projects/[id]/contracts/[contractId]/line-items/[lineItemId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    if (!projectMember || !['admin', 'owner'].includes(projectMember.access)) {
      return NextResponse.json(
        { error: 'Forbidden: You do not have permission to delete line items for this project' },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json({ message: 'Line item deleted successfully' }, { status: 200 });
  } catch (error) {
    console.error('Error in DELETE /api/projects/[id]/contracts/[contractId]/line-items/[lineItemId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
