...
export async function GET(
  ...
  try {
    ...
    console.error('Error in GET /api/projects/[id]/contracts/[contractId]/change-orders:', error);
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    if (!projectMember || !['editor', 'admin', 'owner'].includes(projectMember.access)) {
      return NextResponse.json(
        { error: 'Forbidden: You do not have permission to create change orders for this project' },
        { status: 403 }
      );
    }
    ...
    console.error('Error in POST /api/projects/[id]/contracts/[contractId]/change-orders:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
