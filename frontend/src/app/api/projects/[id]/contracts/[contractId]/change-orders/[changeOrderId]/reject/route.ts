...
export async function POST(
  ...
  try {
    ...
    if (!projectMember || !['admin', 'owner'].includes(projectMember.access)) {
      return NextResponse.json(
        { error: 'Forbidden: You do not have permission to reject change orders for this project' },
        { status: 403 }
      );
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to reject change order', details: updateError.message },
        { status: 400 }
      );
    }
    ...
    console.error('Error in POST /api/projects/[id]/contracts/[contractId]/change-orders/[changeOrderId]/reject:', error);
    return NextResponse.json(
