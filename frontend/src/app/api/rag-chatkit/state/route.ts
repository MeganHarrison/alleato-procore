...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('[RAG-ChatKit State] ⚠️  Backend responded with status', response.status);
      return respondWithOfflinePayload(buildOfflineStateResponse(threadId), `backend-status-${response.status}`);
    }
    ...
    console.error('[RAG-ChatKit State] ❌ Error:', err.message || 'Unknown error');

    if (isBackendOfflineError(error)) {
      console.error('[RAG-ChatKit State] 🔌 Python backend not running, serving offline data.');
      return respondWithOfflinePayload(buildOfflineStateResponse(threadId), 'backend-offline');
    }
