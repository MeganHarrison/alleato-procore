...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      console.warn('[RAG-ChatKit Bootstrap] ⚠️  Backend responded with status', response.status);
      return respondWithOfflinePayload(buildOfflineBootstrapState(), `backend-status-${response.status}`);
    }
    ...
    console.error('[RAG-ChatKit Bootstrap] ❌ Error:', err.message || 'Unknown error');

    if (isBackendOfflineError(error)) {
      console.error('[RAG-ChatKit Bootstrap] 🔌 Python backend not running, serving offline data.');
      return respondWithOfflinePayload(buildOfflineBootstrapState(), 'backend-offline');
    }
