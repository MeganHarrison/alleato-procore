...
export function isBackendOfflineError(error: unknown) {
  ...
  return code === 'ECONNREFUSED' || message.includes('fetch failed') || message.includes('ECONNREFUSED');
}

export function respondWithOfflinePayload<T extends { context?: Record<string, unknown> }>(payload: T, reason: string) {
