...
import { DirectoryService, type DirectoryFilters, type PersonWithDetails, type PersonCreateDTO, type PersonUpdateDTO } from '@/services/directoryService';
...
interface DirectoryContextValue extends DirectoryState {
  ...
  fetchCompany: (projectId: string, companyId: string) => Promise<Company | null>;
  fetchDistributionGroups: (projectId: string) => Promise<void>;
  createUser: (projectId: string, data: PersonCreateDTO) => Promise<PersonWithDetails>;
  updateUser: (projectId: string, userId: string, data: PersonUpdateDTO) => Promise<PersonWithDetails>;
...
export function DirectoryProvider({ children, projectId }: DirectoryProviderProps) {
  ...
  const fetchUsers = useCallback(async (projectId: string, companyId?: string) => {
  ...
  const fetchCompanies = useCallback(async (projectId: string) => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const fetchCompany = useCallback(async (projectId: string, companyId: string): Promise<Company | null> => {
    try {
  ...
  const fetchDistributionGroups = useCallback(async (projectId: string) => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
      const response = await fetch(`/api/projects/${projectId}/directory/groups?include_members=true`);
      if (!response.ok) {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch distribution groups');
    }
  }, [setError]);

  const createUser = useCallback(async (projectId: string, data: PersonCreateDTO): Promise<PersonWithDetails> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const updateUser = useCallback(async (projectId: string, userId: string, data: PersonUpdateDTO): Promise<PersonWithDetails> => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
      const updatedPerson = await directoryService.updatePerson(projectId, userId, data);
      setState(prev => ({
        ...prev,
  ...
  const deactivateUser = useCallback(async (projectId: string, userId: string) => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const reactivateUser = useCallback(async (projectId: string, userId: string) => {
    setState(prev => ({ ...prev, loading: true, error: null }));
    try {
  ...
  const setPagination = useCallback((page: number, type: 'users' | 'companies' | 'groups') => {
    setState(prev => ({
      ...prev,
      pagination: {
        ...prev.pagination,
