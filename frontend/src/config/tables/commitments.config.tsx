...
export function getCommitmentsColumns(
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => formatCurrency(row.getValue("revised_contract_amount")),
    },
    {
...
export function getCommitmentsSummaryCards(
  ...
  const totals = commitments.reduce(
    (acc, commitment) => ({
      ...
      changeOrdersTotal: acc.changeOrdersTotal + (commitment.approved_change_orders || 0),
      revisedAmount: acc.revisedAmount + (commitment.revised_contract_amount || 0),
      balanceToFinish: acc.balanceToFinish + (commitment.balance_to_finish || 0),
    }),
    { originalAmount: 0, changeOrdersTotal: 0, revisedAmount: 0, balanceToFinish: 0 }
  )
