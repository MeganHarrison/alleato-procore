...
* [ ] Categorize into: Daily critical, Nice-to-have, Unnecessary
...
* [ ] List all required Supabase tables

### **1.3 — Organize crawl outputs**

* [ ] Structure results by module in `/procore-crawl/*`
* [ ] Extract unique screens per module.

### **1.4 — Import into design workspace**
...
* [x] Cloudflare R2 (optional for heavy files)
* [x] OpenAI for AI automation
...
### **11.1 - Agents SDK + ChatKit session**

* [ ] Register or provision an Agents SDK agent that exposes the workflows needed for Alleato chat scenarios (meetings, documents, etc.).
* [x] Build a server endpoint that calls `client.beta.chatkit.sessions.create` (or the equivalent `chatkit/sessions` call) using the agent workflow ID and returns `client_secret` with cookie-based user tracking. (`frontend/app/api/chatkit/session/route.ts`)
* [ ] Persist any required secrets/ids (`OPENAI_API_KEY`, `NEXT_PUBLIC_CHATKIT_WORKFLOW_ID`, domain allowlist) and document how to refresh tokens or agents via the SDK.

### **11.2 - Supabase RAG tool**

* [ ] Ensure the `documents` table stores vector embeddings (new column or table) and confirm data syncing processes whenever documents change.
* [x] Add a Supabase RPC or edge function that performs vector search against document embeddings and returns top hits with metadata for the agent. (`frontend/app/api/docs-search/route.ts`)
* [x] Wrap that Supabase tool as a server-side “client tool” so the agent powering ChatKit can request RAG content during conversations. (`frontend/app/chat/page.tsx`)

### **11.3 - Frontend ChatKit integration**

* [x] Create the `/chat` client page that loads `@openai/chatkit-react`, fetches the session endpoint, configures prompts/history, and renders `<ChatKit control={control} />`. (`frontend/app/chat/page.tsx`)
* [x] Wire up the page layout and styles to match the Alleato dashboard while keeping the chat widget responsive. (`frontend/app/chat/page.tsx`)
* [x] Handle client tool hooks (session refresh, tool responses) so the UI displays agent-driven RAG results pulled from Supabase documents. (`frontend/app/chat/page.tsx`)
* [ ] Validate the end-to-end flow (`pnpm dev`): ChatKit loads, the session endpoint returns secrets, the agent calls the Supabase RAG tool, and those results appear in the chat.
