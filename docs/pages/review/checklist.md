...
| Criteria | Status | Notes |
|----------|--------|-------|
| Displays all budget line items for project | ✅ | Powered by `frontend/src/app/(project-mgmt)/[projectId]/budget/page.tsx` via `/api/projects/[id]/budget` |
| Columns: Cost Code, Description, Original Budget, Modifications, Revised Budget, Committed, Direct Costs, Forecast | ✅ | Implemented in `frontend/src/components/budget/budget-table.tsx` |
| Hierarchical display by cost code | ⬜ | Table supports children but API currently returns flat rows; no grouping yet |
| Subtotals by division | ⬜ | Needs aggregation logic (e.g., rollups per division) |
| Grand totals row | ✅ | Sticky totals row rendered in `BudgetTable` |
| Fullscreen mode toggle | ⬜ | Button only logs to console; no layout change/state |
| Filter by cost type | ⬜ | UI button exists but no filtering logic wired |
| Export functionality | ⬜ | Export handlers are stubs |

#### Add Budget Line Item
| Criteria | Status | Notes |
|----------|--------|-------|
| Form for new budget item | ✅ | Sidebar modal in `budget-line-item-modal.tsx` |
| Cost code selector | ✅ | Command palette + `/api/projects/[id]/budget-codes` lookup |
| Original budget amount | ✅ | Amount column captured per row |
| Unit quantities (if unit-based) | ✅ | Qty/UOM/unit cost fields with auto calculation |
| Submit saves to `budget_items` | ✅ | POST `/api/projects/[id]/budget` inserts rows (needs Zod per global criteria) |

#### Budget Modifications
| Criteria | Status | Notes |
|----------|--------|-------|
| Add modification to budget item | ⬜ | UI modal exists but is not wired to Supabase |
| Modification amount (+ or -) | ⬜ | Field present, lacks validation + persistence |
| Description/reason | ⬜ | Text fields exist but unused without backend |
| Saves to `budget_modifications` table | ⬜ | API endpoint not implemented |
